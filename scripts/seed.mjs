...
/**
 * No fixed default passwords exist anywhere in this repository.
 *
 * A committed default is a live credential the moment anyone clones and
 * deploys: the repo tells an attacker the email AND the password. Generating
 * one per seed means the only way to know it is to have run the seed yourself.
 *
 * Alphanumeric only — these end up pasted into URLs and shell commands, and
 * punctuation in a connection string is exactly what breaks it.
 */
function generatePassword() {
  return randomBytes(24).toString('base64').replace(/[^a-zA-Z0-9]/g, '').slice(0, 20)
}

const ADMIN_EMAIL = process.env.ADMIN_EMAIL ?? 'admin@genie.local'
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? generatePassword()
const DEMO_PASSWORD = process.env.DEMO_PASSWORD ?? generatePassword()

const GENERATED = !process.env.ADMIN_PASSWORD || !process.env.DEMO_PASSWORD
...
// Gitignored. The E2E suite reads this rather than embedding a password, and it
// is how you recover the credentials after the console output scrolls away.
writeFileSync(
  path.join(process.cwd(), '.seed-credentials.json'),
  JSON.stringify(
    {
      note: 'Generated by npm run db:seed. Gitignored. Never commit or share.',
      admin: { email: ADMIN_EMAIL, password: ADMIN_PASSWORD },
      business: { email: 'owner@urbanbites.local', password: DEMO_PASSWORD },
      businessTrial: { email: 'owner@urbanthreads.local', password: DEMO_PASSWORD },
    },
    null,
    2,
  ) + '\n',
...
────────────────────────────────────────────────────────────────
${GENERATED
  ? `  These passwords were generated for this seed and written to
  .seed-credentials.json (gitignored, chmod 600).

  Set ADMIN_PASSWORD / DEMO_PASSWORD in .env.local to choose your own.
`
  : `  Using ADMIN_PASSWORD / DEMO_PASSWORD from your environment.
`}────────────────────────────────────────────────────────────────
`)
